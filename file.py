...
def os_demo():
    ...
    print("type of dirs: {}".format(type(dirs)))
    # 打印目录树
    dirs_info = os.scandir()
    for info in dirs_info:
        print("文件名: {}, 路径: {}, inode: {}, 文件夹: {}, 文件: {}, 链接: {}".format(info.name, info.path, info.inode(),
                                                                           info.is_dir(), info.is_file(),
                                                                           info.is_symlink()))
        stats = info.stat()
        print(">>> 访问时间: {}, 修改时间: {}, 模式修改时间: {}, 大小: {}k".format(stats.st_atime, stats.st_ctime, stats.st_mtime,
                                                                   stats.st_size))
        print("{}".format('-' * 10))
    return None
...
def path_demo():
    path = os.getcwd()
    print("当前路径: {}".format(path))
    dir_name = os.path.dirname(path)
    print("文件夹名为: {}".format(dir_name))
...
def pathlib_demo():
    # path = pathlib.Path().home() # C:\Users\comet
    # path = pathlib.Path(__file__) # 文件完整路径+文件名
...
def os_func():
    ...
    dicts = os.environ  # 查看系统环境变量(字典) <class 'os._Environ'>
    strs = os.getenv("Path")  # 读取环境变量, 没有返回None
    ...
    # tups = os.times()
    # 当前进程时间(user用户/system系统/children_user子用户(windows=0)/children_system子系统(windows=0)/elapsed过去时间)
    # nbytes = os.urandom(10)  # Return a string of n random bytes suitable for cryptographic use.
...
def pathlib_func():
    ...
    path.symlink_to("hello.py",
                    target_is_directory=False)  # 创建软链接, 如果目标是目录, 则(windows下)target_is_directory=True (windows失败)
